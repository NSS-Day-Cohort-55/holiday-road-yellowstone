...
export const bizarreDropRender = () => {
  bizarreData().then((obj) => {
    for (const i of obj) {
      ...
      <option class="bizarreListen--${i.id}" value="${i.name}">${i.name}</option>
      `;
    }
  });
};
  ...
  stateNames().then((obj) => {
    for (const thing of obj) {
      document.querySelector(
        "#state-dropdown"
      ).innerHTML += `<option class="state--${thing.id}" value="${thing.abbreviation}">${thing.name}</option>`;
    }
  });
};

export const parkDropRender = (stateCode) => {
  parkData(settings.npsKey, stateCode).then((dataFromPark) => {
    let i = 0;
    document.querySelector("#park-dropdown").innerHTML = "";
    for (const parkObj of dataFromPark.data) {
      document.querySelector(
        "#park-dropdown"
      ).innerHTML += `<option class="park--${parkObj.id}" value="${i}">${parkObj.fullName}</option>`;
      i++;
    }
  });
};
